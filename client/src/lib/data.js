// ===========================
//        USER ROLE
// ===========================
export let role = "superadmin"; // admin | subadmin | superadmin | student

export const facultysData = [
  {
    id: 1,
    facultyId: "FAC-0001",
    username: "johndoe",
    firstName: "John",
    lastName: "Doe",
    department: "CSE",
    name: "John Doe", 
    email: "john@doe.com",
    photo:
      "https://images.pexels.com/photos/2888150/pexels-photo-2888150.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/2888150/pexels-photo-2888150.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "password123",
    phone: "+1 987 654 3210",
    Subjects: ["Math", "Geometry"],
    classes: ["1B", "2A", "3C"],
    address: "123 Main St, Anytown, USA",
    bloodType: "A+",
    dateOfBirth: "1985-06-15",
    sex: "male",
  },
  {
    id: 2,
    facultyId: "FAC-0002",
    username: "janedoe",
    firstName: "Jane",
    lastName: "Doe",
    department: "ECE",
    name: "Jane Doe",
    email: "jane@doe.com",
    photo:
      "https://images.pexels.com/photos/936126/pexels-photo-936126.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/936126/pexels-photo-936126.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3211",
    Subjects: ["Physics", "Chemistry"],
    classes: ["5A", "4B", "3C"],
    address: "234 Oak Ave, Anytown, USA",
    bloodType: "B-",
    dateOfBirth: "1990-11-02",
    sex: "female",
  },
  {
    id: 3,
    facultyId: "FAC-0003",
    username: "mikeg",
    firstName: "Mike",
    lastName: "Geller",
    department: "BIO",
    name: "Mike Geller",
    email: "mike@geller.com",
    photo:
      "https://images.pexels.com/photos/428328/pexels-photo-428328.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/428328/pexels-photo-428328.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3212",
    Subjects: ["Biology"],
    classes: ["5A", "4B", "3C"],
    address: "345 Pine St, Anytown, USA",
    bloodType: "O+",
    dateOfBirth: "1982-03-20",
    sex: "male",
  },
  {
    id: 4,
    facultyId: "FAC-0004",
    username: "jayf",
    firstName: "Jay",
    lastName: "French",
    department: "HIST",
    name: "Jay French",
    email: "jay@gmail.com",
    photo:
      "https://images.pexels.com/photos/1187765/pexels-photo-1187765.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/1187765/pexels-photo-1187765.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3213",
    Subjects: ["History"],
    classes: ["5A", "4B", "3C"],
    address: "456 Cedar Lane, Anytown, USA",
    bloodType: "AB+",
    dateOfBirth: "1978-09-10",
    sex: "male",
  },
  {
    id: 5,
    facultyId: "FAC-0005",
    username: "janes",
    firstName: "Jane",
    lastName: "Smith",
    department: "ARTS",
    name: "Jane Smith",
    email: "jane@gmail.com",
    photo:
      "https://images.pexels.com/photos/1102341/pexels-photo-1102341.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/1102341/pexels-photo-1102341.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3214",
    Subjects: ["Music", "History"],
    classes: ["5A", "4B", "3C"],
    address: "567 Birch Rd, Anytown, USA",
    bloodType: "A-",
    dateOfBirth: "1992-12-05",
    sex: "female",
  },
  {
    id: 6,
    facultyId: "FAC-0006",
    username: "annas",
    firstName: "Anna",
    lastName: "Santiago",
    department: "ECE",
    name: "Anna Santiago",
    email: "anna@gmail.com",
    photo:
      "https://images.pexels.com/photos/712513/pexels-photo-712513.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/712513/pexels-photo-712513.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3215",
    Subjects: ["Physics"],
    classes: ["5A", "4B", "3C"],
    address: "678 Elm St, Anytown, USA",
    bloodType: "O-",
    dateOfBirth: "1988-04-18",
    sex: "female",
  },
  {
    id: 7,
    facultyId: "FAC-0007",
    username: "allenb",
    firstName: "Allen",
    lastName: "Black",
    department: "ENG",
    name: "Allen Black",
    email: "allen@black.com",
    photo:
      "https://images.pexels.com/photos/1438081/pexels-photo-1438081.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/1438081/pexels-photo-1438081.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3216",
    Subjects: ["English", "Spanish"],
    classes: ["5A", "4B", "3C"],
    address: "789 Spruce Way, Anytown, USA",
    bloodType: "B+",
    dateOfBirth: "1980-02-25",
    sex: "male",
  },
  {
    id: 8,
    facultyId: "FAC-0008",
    username: "opheliac",
    firstName: "Ophelia",
    lastName: "Castro",
    department: "MATH",
    name: "Ophelia Castro",
    email: "ophelia@castro.com",
    photo:
      "https://images.pexels.com/photos/1036623/pexels-photo-1036623.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/1036623/pexels-photo-1036623.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3217",
    Subjects: ["Math", "Geometry"],
    classes: ["5A", "4B", "3C"],
    address: "890 Willow Rd, Anytown, USA",
    bloodType: "A+",
    dateOfBirth: "1994-07-30",
    sex: "female",
  },
  {
    id: 9,
    facultyId: "FAC-0009",
    username: "derekb",
    firstName: "Derek",
    lastName: "Briggs",
    department: "LIT",
    name: "Derek Briggs",
    email: "derek@briggs.com",
    photo:
      "https://images.pexels.com/photos/842980/pexels-photo-842980.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/842980/pexels-photo-842980.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3218",
    Subjects: ["Literature", "English"],
    classes: ["5A", "4B", "3C"],
    address: "901 Aspen Ct, Anytown, USA",
    bloodType: "O+",
    dateOfBirth: "1975-01-07",
    sex: "male",
  },
  {
    id: 10,
    facultyId: "FAC-0010",
    username: "johng",
    firstName: "John",
    lastName: "Glover",
    department: "BIO",
    name: "John Glover",
    email: "john@glover.com",
    photo:
      "https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg?auto=compress&cs=tinysrgb&w=1200",
    img: "https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg?auto=compress&cs=tinysrgb&w=1200",
    password: "",
    phone: "+1 987 654 3219",
    Subjects: ["Biology"],
    classes: ["5A", "4B", "3C"],
    address: "1010 Maple Ave, Anytown, USA",
    bloodType: "B-",
    dateOfBirth: "1987-08-22",
    sex: "male",
  },
];

// ===========================
//      STUDENTS DATA
// 15 students — FULL
// ===========================
export const studentsData = [
  // -------- CSE ---------
  {
    enrolmentNo: "2025110001",
    rollNo: "18349201",
    name: "Arjun Mehta",
    email: "arjun.mehta@college.com",
    photo: "https://randomuser.me/api/portraits/men/11.jpg",
    phone: "9876543211",
    semester: 4,
    department: "CSE",
    class: "A",
    address: "45 Lake View Road, Patna",
  },
  {
    enrolmentNo: "2025110002",
    rollNo: "18349202",
    name: "Neha Singh",
    email: "neha.singh@college.com",
    photo: "https://randomuser.me/api/portraits/women/12.jpg",
    phone: "9876543212",
    semester: 3,
    department: "CSE",
    class: "B",
    address: "12 Green Park, Delhi",
  },
  {
    enrolmentNo: "2025110003",
    rollNo: "18349203",
    name: "Rohit Verma",
    email: "rohit.verma@college.com",
    photo: "https://randomuser.me/api/portraits/men/13.jpg",
    phone: "9876543213",
    semester: 2,
    department: "CSE",
    class: "C",
    address: "7 Sunrise Colony, Lucknow",
  },
  {
    enrolmentNo: "2025110004",
    rollNo: "18349204",
    name: "Simran Kaur",
    email: "simran.kaur@college.com",
    photo: "https://randomuser.me/api/portraits/women/14.jpg",
    phone: "9876543214",
    semester: 1,
    department: "CSE",
    class: "A",
    address: "89 River View, Chandigarh",
  },
  {
    enrolmentNo: "2025110005",
    rollNo: "18349205",
    name: "Aditya Kumar",
    email: "aditya.kumar@college.com",
    photo: "https://randomuser.me/api/portraits/men/15.jpg",
    phone: "9876543215",
    semester: 3,
    department: "CSE",
    class: "B",
    address: "54 Garden Estate, Noida",
  },

  // -------- ECE ---------
  {
    enrolmentNo: "2025110006",
    rollNo: "28457201",
    name: "Shruti Sharma",
    email: "shruti.sharma@college.com",
    photo: "https://randomuser.me/api/portraits/women/21.jpg",
    phone: "9876543221",
    semester: 4,
    department: "ECE",
    class: "A",
    address: "31 Maple Street, Jaipur",
  },
  {
    enrolmentNo: "2025110007",
    rollNo: "28457202",
    name: "Kunal Joshi",
    email: "kunal.joshi@college.com",
    photo: "https://randomuser.me/api/portraits/men/22.jpg",
    phone: "9876543222",
    semester: 2,
    department: "ECE",
    class: "B",
    address: "9 Hill Top, Ahmedabad",
  },
  {
    enrolmentNo: "2025110008",
    rollNo: "28457203",
    name: "Priya Nair",
    email: "priya.nair@college.com",
    photo: "https://randomuser.me/api/portraits/women/23.jpg",
    phone: "9876543223",
    semester: 1,
    department: "ECE",
    class: "C",
    address: "88 Palm Avenue, Kochi",
  },
  {
    enrolmentNo: "2025110009",
    rollNo: "28457204",
    name: "Sahil Khan",
    email: "sahil.khan@college.com",
    photo: "https://randomuser.me/api/portraits/men/24.jpg",
    phone: "9876543224",
    semester: 3,
    department: "ECE",
    class: "A",
    address: "101 Royal Enclave, Mumbai",
  },
  {
    enrolmentNo: "2025110010",
    rollNo: "28457205",
    name: "Ishita Roy",
    email: "ishita.roy@college.com",
    photo: "https://randomuser.me/api/portraits/women/25.jpg",
    phone: "9876543225",
    semester: 4,
    department: "ECE",
    class: "B",
    address: "44 Lotus Garden, Kolkata",
  },

  // -------- MECH ---------
  {
    enrolmentNo: "2025110011",
    rollNo: "39568101",
    name: "Mohit Rana",
    email: "mohit.rana@college.com",
    photo: "https://randomuser.me/api/portraits/men/31.jpg",
    phone: "9876543231",
    semester: 3,
    department: "MECH",
    class: "C",
    address: "67 Steel Colony, Jamshedpur",
  },
  {
    enrolmentNo: "2025110012",
    rollNo: "39568102",
    name: "Ananya Gupta",
    email: "ananya.gupta@college.com",
    photo: "https://randomuser.me/api/portraits/women/32.jpg",
    phone: "9876543232",
    semester: 1,
    department: "MECH",
    class: "A",
    address: "5 Pearl Tower, Bhopal",
  },
  {
    enrolmentNo: "2025110013",
    rollNo: "39568103",
    name: "Deepak Rawat",
    email: "deepak.rawat@college.com",
    photo: "https://randomuser.me/api/portraits/men/33.jpg",
    phone: "9876543233",
    semester: 4,
    department: "MECH",
    class: "B",
    address: "72 Engine Road, Dehradun",
  },
  {
    enrolmentNo: "2025110014",
    rollNo: "39568104",
    name: "Riya Malhotra",
    email: "riya.malhotra@college.com",
    photo: "https://randomuser.me/api/portraits/women/34.jpg",
    phone: "9876543234",
    semester: 2,
    department: "MECH",
    class: "C",
    address: "90 Sunset Block, Indore",
  },
  {
    enrolmentNo: "2025110015",
    rollNo: "39568105",
    name: "Yash Patel",
    email: "yash.patel@college.com",
    photo: "https://randomuser.me/api/portraits/men/35.jpg",
    phone: "9876543235",
    semester: 4,
    department: "MECH",
    class: "A",
    address: "23 Valley Road, Surat",
  },
];

// ===========================
//      PARENTS DATA
// (original unchanged)
// ===========================
export const parentsData = [
  {
    id: 1,
    name: "John Doe",
    students: ["Sarah Brewer"],
    email: "john@doe.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 2,
    name: "Jane Doe",
    students: ["Cecilia Bradley"],
    email: "jane@doe.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 3,
    name: "Mike Geller",
    students: ["Fanny Caldwell"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 4,
    name: "Jay French",
    students: ["Mollie Fitzgerald", "Ian Bryant"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 5,
    name: "Jane Smith",
    students: ["Mable Harvey"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 6,
    name: "Anna Santiago",
    students: ["Joel Lambert"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 7,
    name: "Allen Black",
    students: ["Carrie Tucker", "Lilly Underwood"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 8,
    name: "Ophelia Castro",
    students: ["Alexander Blair"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 9,
    name: "Derek Briggs",
    students: ["Susan Webster", "Maude Stone"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 10,
    name: "John Glover",
    students: ["Stella Scott"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
];

// ===========================
//      Subjects DATA
// ===========================
export const SubjectsData = [
  { id: 1, name: "Math", facultys: ["Alice Phelps", "Russell Davidson"] },
  { id: 2, name: "English", facultys: ["Manuel Becker", "Eddie Chavez"] },
  { id: 3, name: "Physics", facultys: ["Lola Newman", "Darrell Delgado"] },
  { id: 4, name: "Chemistry", facultys: ["Nathan Kelly", "Benjamin Snyder"] },
  { id: 5, name: "Biology", facultys: ["Alma Benson", "Lina Collier"] },
  { id: 6, name: "History", facultys: ["Hannah Bowman", "Betty Obrien"] },
  { id: 7, name: "Geography", facultys: ["Lora French", "Sue Brady"] },
  { id: 8, name: "Art", facultys: ["Harriet Alvarado", "Mayme Keller"] },
  { id: 9, name: "Music", facultys: ["Gertrude Roy", "Rosa Singleton"] },
  { id: 10, name: "Literature", facultys: ["Effie Lynch", "Brett Flowers"] },
];

// ===========================
//      CLASSES DATA
// ===========================
export const classesData = [
  { id: 1, name: "1A", capacity: 20, grade: 1, supervisor: "Joseph Padilla" },
  { id: 2, name: "2B", capacity: 22, grade: 2, supervisor: "Blake Joseph" },
  { id: 3, name: "3C", capacity: 20, grade: 3, supervisor: "Tom Bennett" },
  { id: 4, name: "4B", capacity: 18, grade: 4, supervisor: "Aaron Collins" },
  { id: 5, name: "5A", capacity: 16, grade: 5, supervisor: "Iva Frank" },
  { id: 6, name: "5B", capacity: 20, grade: 5, supervisor: "Leila Santos" },
  { id: 7, name: "7A", capacity: 18, grade: 7, supervisor: "Carrie Walton" },
  { id: 8, name: "6B", capacity: 22, grade: 6, supervisor: "Christopher Butler" },
  { id: 9, name: "6C", capacity: 18, grade: 6, supervisor: "Marc Miller" },
  { id: 10, name: "6D", capacity: 20, grade: 6, supervisor: "Ophelia Marsh" },
];

// ===========================
//      LESSONS DATA
// ===========================
export const lessonsData = [
  { id: 1, subject: "Math", class: "1A", faculty: "Tommy Wise" },
  { id: 2, subject: "English", class: "2A", faculty: "Rhoda Frank" },
  { id: 3, subject: "Science", class: "3A", faculty: "Della Dunn" },
  { id: 4, subject: "Social Studies", class: "1B", faculty: "Bruce Rodriguez" },
  { id: 5, subject: "Art", class: "4A", faculty: "Birdie Butler" },
  { id: 6, subject: "Music", class: "5A", faculty: "Bettie Oliver" },
  { id: 7, subject: "History", class: "6A", faculty: "Herman Howard" },
  { id: 8, subject: "Geography", class: "6B", faculty: "Lucinda Thomas" },
  { id: 9, subject: "Physics", class: "6C", faculty: "Ronald Roberts" },
  { id: 10, subject: "Chemistry", class: "4B", faculty: "Julia Pittman" },
];

// ===========================
//      EXAMS DATA
// ===========================
export const examsData = [
  { id: 1, subject: "Math", class: "1A", faculty: "Martha Morris", date: "2025-11-13" },
  { id: 2, subject: "English", class: "2A", faculty: "Randall Garcia", date: "2025-11-13" },
  { id: 3, subject: "Science", class: "3A", faculty: "Myrtie Scott", date: "2025-11-13" },
  { id: 4, subject: "Social Studies", class: "1B", faculty: "Alvin Swanson", date: "2025-11-13" },
  { id: 5, subject: "Art", class: "4A", faculty: "Mabelle Wallace", date: "2025-11-13" },
  { id: 6, subject: "Music", class: "5A", faculty: "Dale Thompson", date: "2025-11-13" },
  { id: 7, subject: "History", class: "6A", faculty: "Allie Conner", date: "2025-11-13" },
  { id: 8, subject: "Geography", class: "6B", faculty: "Hunter Fuller", date: "2025-11-13" },
  { id: 9, subject: "Physics", class: "7A", faculty: "Lois Lindsey", date: "2025-11-13" },
  { id: 10, subject: "Chemistry", class: "8A", faculty: "Vera Soto", date: "2025-11-13" },
];

// ===========================
//      ASSIGNMENTS DATA
// ===========================
export const assignmentsData = [
  { id: 1, subject: "Math", class: "1A", faculty: "Anthony Boone", dueDate: "2025-11-13" },
  { id: 2, subject: "English", class: "2A", faculty: "Clifford Bowen", dueDate: "2025-11-13" },
  { id: 3, subject: "Science", class: "3A", faculty: "Catherine Malone", dueDate: "2025-11-13" },
  { id: 4, subject: "Social Studies", class: "1B", faculty: "Willie Medina", dueDate: "2025-11-13" },
  { id: 5, subject: "Art", class: "4A", faculty: "Jose Ruiz", dueDate: "2025-11-13" },
  { id: 6, subject: "Music", class: "5A", faculty: "Katharine Owens", dueDate: "2025-11-13" },
  { id: 7, subject: "History", class: "6A", faculty: "Shawn Norman", dueDate: "2025-11-13" },
  { id: 8, subject: "Geography", class: "6B", faculty: "Don Holloway", dueDate: "2025-11-13" },
  { id: 9, subject: "Physics", class: "7A", faculty: "Franklin Gregory", dueDate: "2025-11-13" },
  { id: 10, subject: "Chemistry", class: "8A", faculty: "Danny Nguyen", dueDate: "2025-11-13" },
];

// ===========================
//      RESULTS DATA
// ===========================
export const resultsData = [
  {
    id: 1,
    subject: "Math",
    class: "1A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 2,
    subject: "English",
    class: "2A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 3,
    subject: "Science",
    class: "3A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 4,
    subject: "Social Studies",
    class: "1B",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 5,
    subject: "Art",
    class: "4A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 6,
    subject: "Music",
    class: "5A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 7,
    subject: "History",
    class: "6A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 8,
    subject: "Geography",
    class: "6B",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 9,
    subject: "Physics",
    class: "7A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
  {
    id: 10,
    subject: "Chemistry",
    class: "8A",
    faculty: "John Doe",
    student: "John Doe",
    date: "2025-11-13",
    type: "exam",
    score: 90,
  },
];

// ===========================
//        EVENTS DATA
// ===========================
export const eventsData = [
  {
    id: 1,
    title: "Lake Trip",
    class: "1A",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 2,
    title: "Picnic",
    class: "2A",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 3,
    title: "Beach Trip",
    class: "3A",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 4,
    title: "Museum Trip",
    class: "4A",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 5,
    title: "Music Concert",
    class: "5A",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 6,
    title: "Magician Show",
    class: "1B",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 7,
    title: "Lake Trip",
    class: "2B",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 8,
    title: "Cycling Race",
    class: "3B",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 9,
    title: "Art Exhibition",
    class: "4B",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
  {
    id: 10,
    title: "Sports Tournament",
    class: "5B",
    date: "2025-11-13",
    startTime: "10:00",
    endTime: "11:00",
  },
];

// ===========================
//      CALENDAR EVENTS
// ===========================
export const calendarEvents = [
  {
    title: "Math",
    allDay: false,
    start: new Date(2025, 10, 13, 8, 0),
    end: new Date(2025, 10, 13, 8, 45),
  },
  {
    title: "English",
    allDay: false,
    start: new Date(2025, 10, 13, 9, 0),
    end: new Date(2025, 10, 13, 9, 45),
  },
  {
    title: "Biology",
    allDay: false,
    start: new Date(2025, 10, 13, 10, 0),
    end: new Date(2025, 10, 13, 10, 45),
  },
  {
    title: "Physics",
    allDay: false,
    start: new Date(2025, 10, 13, 11, 0),
    end: new Date(2025, 10, 13, 11, 45),
  },
  {
    title: "Chemistry",
    allDay: false,
    start: new Date(2025, 10, 13, 13, 0),
    end: new Date(2025, 10, 13, 13, 45),
  },
  {
    title: "History",
    allDay: false,
    start: new Date(2025, 10, 13, 14, 0),
    end: new Date(2025, 10, 13, 14, 45),
  },
];


// ===========================
//     FINANCE DATA MODELS
// ===========================

// Fee Categories
export const feeCategories = [
  { id: 1, name: "Tuition Fee", code: "TUITION", isActive: true },
  { id: 2, name: "Library Fee", code: "LIBRARY", isActive: true },
  { id: 3, name: "Laboratory Fee", code: "LAB", isActive: true },
  { id: 4, name: "Sports Fee", code: "SPORTS", isActive: true },
  { id: 5, name: "Development Fee", code: "DEVELOPMENT", isActive: true },
  { id: 6, name: "Examination Fee", code: "EXAM", isActive: true },
  { id: 7, name: "Hostel Fee", code: "HOSTEL", isActive: true },
  { id: 8, name: "Transportation Fee", code: "TRANSPORT", isActive: true },
];

// Fee Structure by Department and Semester
export const feeStructures = [
  // CSE Department
  {
    id: 1,
    departmentCode: "CSE",
    semester: 1,
    academicYear: "2024-25",
    fees: [
      { categoryId: 1, amount: 45000 }, // Tuition
      { categoryId: 2, amount: 2000 },  // Library
      { categoryId: 3, amount: 5000 },  // Lab
      { categoryId: 4, amount: 1500 },  // Sports
      { categoryId: 5, amount: 3000 },  // Development
      { categoryId: 6, amount: 2500 },  // Exam
    ],
    totalAmount: 59000,
    isActive: true,
  },
  {
    id: 2,
    departmentCode: "ECE",
    semester: 1,
    academicYear: "2024-25",
    fees: [
      { categoryId: 1, amount: 42000 },
      { categoryId: 2, amount: 2000 },
      { categoryId: 3, amount: 4500 },
      { categoryId: 4, amount: 1500 },
      { categoryId: 5, amount: 3000 },
      { categoryId: 6, amount: 2500 },
    ],
    totalAmount: 55500,
    isActive: true,
  },
  {
    id: 3,
    departmentCode: "ME",
    semester: 1,
    academicYear: "2024-25",
    fees: [
      { categoryId: 1, amount: 40000 },
      { categoryId: 2, amount: 2000 },
      { categoryId: 3, amount: 4000 },
      { categoryId: 4, amount: 1500 },
      { categoryId: 5, amount: 3000 },
      { categoryId: 6, amount: 2500 },
    ],
    totalAmount: 53000,
    isActive: true,
  },
];

// Student Fee Records
export const studentFeeRecords = [
  {
    id: 1,
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    department: "CSE",
    semester: 4,
    academicYear: "2024-25",
    feeStructureId: 1,
    totalAmount: 59000,
    paidAmount: 59000,
    pendingAmount: 0,
    status: "Paid",
    dueDate: "2024-08-15",
    paymentDate: "2024-08-10",
    paymentMethod: "Online",
    transactionId: "TXN001234567",
    receipts: ["RCP001", "RCP002"],
  },
  {
    id: 2,
    studentEnrollment: "2025110002",
    studentName: "Neha Singh",
    department: "CSE",
    semester: 3,
    academicYear: "2024-25",
    feeStructureId: 1,
    totalAmount: 59000,
    paidAmount: 30000,
    pendingAmount: 29000,
    status: "Partial",
    dueDate: "2024-08-15",
    paymentDate: "2024-07-20",
    paymentMethod: "Cash",
    transactionId: "TXN001234568",
    receipts: ["RCP003"],
  },
  {
    id: 3,
    studentEnrollment: "2025110003",
    studentName: "Rohit Verma",
    department: "CSE",
    semester: 2,
    academicYear: "2024-25",
    feeStructureId: 1,
    totalAmount: 59000,
    paidAmount: 0,
    pendingAmount: 59000,
    status: "Pending",
    dueDate: "2024-08-15",
    paymentDate: null,
    paymentMethod: null,
    transactionId: null,
    receipts: [],
  },
  {
    id: 4,
    studentEnrollment: "2025110006",
    studentName: "Shruti Sharma",
    department: "ECE",
    semester: 4,
    academicYear: "2024-25",
    feeStructureId: 2,
    totalAmount: 55500,
    paidAmount: 55500,
    pendingAmount: 0,
    status: "Paid",
    dueDate: "2024-08-15",
    paymentDate: "2024-08-12",
    paymentMethod: "Online",
    transactionId: "TXN001234569",
    receipts: ["RCP004"],
  },
  {
    id: 5,
    studentEnrollment: "2025110011",
    studentName: "Mohit Rana",
    department: "MECH",
    semester: 3,
    academicYear: "2024-25",
    feeStructureId: 3,
    totalAmount: 53000,
    paidAmount: 25000,
    pendingAmount: 28000,
    status: "Partial",
    dueDate: "2024-08-15",
    paymentDate: "2024-08-05",
    paymentMethod: "Bank Transfer",
    transactionId: "TXN001234570",
    receipts: ["RCP005"],
  },
];

// Expense Categories
export const expenseCategories = [
  { id: 1, name: "Faculty Salaries", code: "FACULTY_SALARY", type: "recurring" },
  { id: 2, name: "Staff Salaries", code: "STAFF_SALARY", type: "recurring" },
  { id: 3, name: "Utilities", code: "UTILITIES", type: "recurring" },
  { id: 4, name: "Maintenance", code: "MAINTENANCE", type: "variable" },
  { id: 5, name: "Equipment Purchase", code: "EQUIPMENT", type: "one-time" },
  { id: 6, name: "Library Books", code: "LIBRARY_BOOKS", type: "variable" },
  { id: 7, name: "Office Supplies", code: "OFFICE_SUPPLIES", type: "variable" },
  { id: 8, name: "Marketing", code: "MARKETING", type: "variable" },
  { id: 9, name: "Events & Activities", code: "EVENTS", type: "variable" },
  { id: 10, name: "Insurance", code: "INSURANCE", type: "recurring" },
];

// Expense Records
export const expenseRecords = [
  {
    id: 1,
    description: "Faculty Salaries - October 2024",
    categoryId: 1,
    amount: 850000,
    date: "2024-10-31",
    paymentMethod: "Bank Transfer",
    approvedBy: "Dr. Amit Verma",
    status: "Paid",
    invoiceNumber: "INV-2024-001",
    vendor: "Internal",
  },
  {
    id: 2,
    description: "Laboratory Equipment - Chemistry Department",
    categoryId: 5,
    amount: 125000,
    date: "2024-10-28",
    paymentMethod: "Cheque",
    approvedBy: "Dr. Amit Verma",
    status: "Paid",
    invoiceNumber: "INV-2024-002",
    vendor: "Scientific Instruments Ltd.",
  },
  {
    id: 3,
    description: "Electricity Bill - October 2024",
    categoryId: 3,
    amount: 45000,
    date: "2024-10-25",
    paymentMethod: "Online",
    approvedBy: "Finance Officer",
    status: "Paid",
    invoiceNumber: "INV-2024-003",
    vendor: "State Electricity Board",
  },
  {
    id: 4,
    description: "Library Books Purchase",
    categoryId: 6,
    amount: 35000,
    date: "2024-10-20",
    paymentMethod: "Bank Transfer",
    approvedBy: "Dr. Amit Verma",
    status: "Pending",
    invoiceNumber: "INV-2024-004",
    vendor: "Academic Publishers",
  },
  {
    id: 5,
    description: "Annual College Fest",
    categoryId: 9,
    amount: 80000,
    date: "2024-10-15",
    paymentMethod: "Cash",
    approvedBy: "Dr. Amit Verma",
    status: "Paid",
    invoiceNumber: "INV-2024-005",
    vendor: "Event Management Co.",
  },
];

// Budget Planning
export const budgetPlanning = [
  {
    id: 1,
    categoryId: 1,
    plannedAmount: 10200000, // Annual faculty salaries
    spentAmount: 8500000,
    remainingAmount: 1700000,
    budgetPeriod: "2024-25",
    variance: -200000, // Over budget
  },
  {
    id: 2,
    categoryId: 2,
    plannedAmount: 3600000, // Annual staff salaries
    spentAmount: 3000000,
    remainingAmount: 600000,
    budgetPeriod: "2024-25",
    variance: 0,
  },
  {
    id: 3,
    categoryId: 3,
    plannedAmount: 540000, // Annual utilities
    spentAmount: 450000,
    remainingAmount: 90000,
    budgetPeriod: "2024-25",
    variance: 90000, // Under budget
  },
  {
    id: 4,
    categoryId: 5,
    plannedAmount: 1500000, // Equipment
    spentAmount: 1250000,
    remainingAmount: 250000,
    budgetPeriod: "2024-25",
    variance: 250000,
  },
];

// Financial Summary Data
export const financialSummary = {
  totalRevenue: 15750000, // From fee collections
  totalExpenses: 13200000,
  netIncome: 2550000,
  totalPendingFees: 1240000,
  totalStudents: 2550,
  averageFeePerStudent: 55000,
  collectionRate: 92.1, // Percentage
  monthlyRevenue: [
    { month: "Apr", amount: 2100000 },
    { month: "May", amount: 1950000 },
    { month: "Jun", amount: 2200000 },
    { month: "Jul", amount: 2350000 },
    { month: "Aug", amount: 2150000 },
    { month: "Sep", amount: 1800000 },
    { month: "Oct", amount: 1950000 },
    { month: "Nov", amount: 1250000 },
  ],
  monthlyExpenses: [
    { month: "Apr", amount: 1100000 },
    { month: "May", amount: 1050000 },
    { month: "Jun", amount: 1200000 },
    { month: "Jul", amount: 1150000 },
    { month: "Aug", amount: 1300000 },
    { month: "Sep", amount: 1000000 },
    { month: "Oct", amount: 1400000 },
    { month: "Nov", amount: 1000000 },
  ],
  departmentWiseRevenue: [
    { department: "CSE", revenue: 6490000, students: 110 },
    { department: "ECE", revenue: 4995000, students: 90 },
    { department: "ME", revenue: 2120000, students: 40 },
    { department: "CE", revenue: 1590000, students: 30 },
    { department: "EE", revenue: 555000, students: 10 },
  ],
};

// ===========================
//      FEE PAYMENT DATA
// ===========================

// Current Semester Registration Status
export const semesterRegistrations = [
  {
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    semester: 4,
    academicYear: "2024-25",
    department: "CSE",
    registrationStatus: "Completed",
    registrationDate: "2024-07-15",
    subjects: ["Data Structures", "Computer Networks", "Software Engineering", "Database Systems"],
    isHosteler: false,
    hostelBlock: null,
    roomNumber: null,
  },
  {
    studentEnrollment: "2025110002",
    studentName: "Neha Singh",
    semester: 3,
    academicYear: "2024-25",
    department: "CSE",
    registrationStatus: "Completed",
    registrationDate: "2024-07-20",
    subjects: ["Object Oriented Programming", "Digital Logic", "Computer Organization", "Mathematics-III"],
    isHosteler: true,
    hostelBlock: "Girls Hostel A",
    roomNumber: "GA-201",
  },
  {
    studentEnrollment: "2025110003",
    studentName: "Rohit Verma",
    semester: 2,
    academicYear: "2024-25",
    department: "CSE",
    registrationStatus: "Pending",
    registrationDate: null,
    subjects: [],
    isHosteler: true,
    hostelBlock: "Boys Hostel B",
    roomNumber: "BB-105",
  },
  {
    studentEnrollment: "2025110006",
    studentName: "Shruti Sharma",
    semester: 4,
    academicYear: "2024-25",
    department: "ECE",
    registrationStatus: "Completed",
    registrationDate: "2024-07-18",
    subjects: ["Microprocessors", "Signal Processing", "Communication Systems", "Control Systems"],
    isHosteler: true,
    hostelBlock: "Girls Hostel B",
    roomNumber: "GB-301",
  },
  {
    studentEnrollment: "2025110011",
    studentName: "Mohit Rana",
    semester: 3,
    academicYear: "2024-25",
    department: "MECH",
    registrationStatus: "Completed",
    registrationDate: "2024-07-25",
    subjects: ["Thermodynamics", "Fluid Mechanics", "Manufacturing Processes", "Machine Design"],
    isHosteler: false,
    hostelBlock: null,
    roomNumber: null,
  },
];

// Fee Payment Structure for Current Semester
export const currentSemesterFees = [
  {
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    semester: 4,
    department: "CSE",
    academicYear: "2024-25",
    isHosteler: false,
    registrationCompleted: true,
    fees: {
      tuitionFee: 45000,
      libraryFee: 2000,
      labFee: 5000,
      sportsFee: 1500,
      developmentFee: 3000,
      examFee: 2500,
      hostelFee: 0, // Day scholar
      messFee: 0,
      total: 59000,
    },
    paymentStatus: "Pending",
    paidAmount: 0,
    pendingAmount: 59000,
    dueDate: "2024-12-15",
    lateFeeApplicable: false,
    lateFeeAmount: 0,
  },
  {
    studentEnrollment: "2025110002",
    studentName: "Neha Singh",
    semester: 3,
    department: "CSE",
    academicYear: "2024-25",
    isHosteler: true,
    registrationCompleted: true,
    fees: {
      tuitionFee: 45000,
      libraryFee: 2000,
      labFee: 5000,
      sportsFee: 1500,
      developmentFee: 3000,
      examFee: 2500,
      hostelFee: 25000, // Hosteler
      messFee: 18000,
      total: 102000,
    },
    paymentStatus: "Partial",
    paidAmount: 50000,
    pendingAmount: 52000,
    dueDate: "2024-12-15",
    lateFeeApplicable: true,
    lateFeeAmount: 2000,
  },
  {
    studentEnrollment: "2025110003",
    studentName: "Rohit Verma",
    semester: 2,
    department: "CSE",
    academicYear: "2024-25",
    isHosteler: true,
    registrationCompleted: false,
    fees: {
      tuitionFee: 45000,
      libraryFee: 2000,
      labFee: 5000,
      sportsFee: 1500,
      developmentFee: 3000,
      examFee: 2500,
      hostelFee: 25000,
      messFee: 18000,
      total: 102000,
    },
    paymentStatus: "Not Eligible", // Registration not completed
    paidAmount: 0,
    pendingAmount: 102000,
    dueDate: "2024-12-15",
    lateFeeApplicable: false,
    lateFeeAmount: 0,
  },
  {
    studentEnrollment: "2025110006",
    studentName: "Shruti Sharma",
    semester: 4,
    department: "ECE",
    academicYear: "2024-25",
    isHosteler: true,
    registrationCompleted: true,
    fees: {
      tuitionFee: 42000,
      libraryFee: 2000,
      labFee: 4500,
      sportsFee: 1500,
      developmentFee: 3000,
      examFee: 2500,
      hostelFee: 25000,
      messFee: 18000,
      total: 98500,
    },
    paymentStatus: "Paid",
    paidAmount: 98500,
    pendingAmount: 0,
    dueDate: "2024-12-15",
    lateFeeApplicable: false,
    lateFeeAmount: 0,
  },
  {
    studentEnrollment: "2025110011",
    studentName: "Mohit Rana",
    semester: 3,
    department: "MECH",
    academicYear: "2024-25",
    isHosteler: false,
    registrationCompleted: true,
    fees: {
      tuitionFee: 40000,
      libraryFee: 2000,
      labFee: 4000,
      sportsFee: 1500,
      developmentFee: 3000,
      examFee: 2500,
      hostelFee: 0,
      messFee: 0,
      total: 53000,
    },
    paymentStatus: "Pending",
    paidAmount: 0,
    pendingAmount: 53000,
    dueDate: "2024-12-15",
    lateFeeApplicable: false,
    lateFeeAmount: 0,
  },
];

// Payment History
export const paymentHistory = [
  // Current Student's Payment History (Arjun Mehta - 2025110001)
  {
    id: "PAY001",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 45000,
    paymentDate: "2024-08-15",
    paymentMethod: "UPI",
    transactionId: "UPI789123456",
    semester: 1,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-001",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
    feeBreakdown: {
      tuitionFee: 35000,
      libraryFee: 2000,
      labFee: 4000,
      sportsFee: 1500,
      developmentFee: 2000,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 0,
      total: 45000
    }
  },
  {
    id: "PAY002",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 48000,
    paymentDate: "2024-01-20",
    paymentMethod: "Net Banking",
    transactionId: "NB456789123",
    semester: 2,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-002",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
    feeBreakdown: {
      tuitionFee: 37000,
      libraryFee: 2000,
      labFee: 4500,
      sportsFee: 1500,
      developmentFee: 2500,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 0,
      total: 48000
    }
  },
  {
    id: "PAY003",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 50000,
    paymentDate: "2024-07-10",
    paymentMethod: "Debit Card",
    transactionId: "DC789456123",
    semester: 3,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-003",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
    feeBreakdown: {
      tuitionFee: 40000,
      libraryFee: 2000,
      labFee: 4000,
      sportsFee: 1500,
      developmentFee: 2000,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 0,
      total: 50000
    }
  },
  {
    id: "PAY004",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 52000,
    paymentDate: "2024-11-05",
    paymentMethod: "UPI",
    transactionId: "UPI654321789",
    semester: 4,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-004",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee", "Late Fee"],
    feeBreakdown: {
      tuitionFee: 40000,
      libraryFee: 2000,
      labFee: 5000,
      sportsFee: 1500,
      developmentFee: 2500,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 500,
      total: 52000
    }
  },
  // Other Students' Payment History
  {
    id: "PAY005",
    studentEnrollment: "2025110002",
    studentName: "Neha Singh",
    amount: 50000,
    paymentDate: "2024-11-01",
    paymentMethod: "Online - UPI",
    transactionId: "UPI789123456",
    semester: 3,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-005",
    status: "Completed",
    paidFor: ["Tuition Fee (Partial)", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
  },
  {
    id: "PAY006",
    studentEnrollment: "2025110006",
    studentName: "Shruti Sharma",
    amount: 98500,
    paymentDate: "2024-10-28",
    paymentMethod: "Online - Net Banking",
    transactionId: "NB456789123",
    semester: 4,
    academicYear: "2024-25",
    receiptNumber: "RCP-2024-006",
    status: "Completed",
    paidFor: ["All Fees (Complete Payment)"],
  },
  {
    id: "PAY007",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 47000,
    paymentDate: "2023-08-20",
    paymentMethod: "Online - UPI",
    transactionId: "UPI987654321",
    semester: 1,
    academicYear: "2023-24",
    receiptNumber: "RCP-2023-001",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
  },
  {
    id: "PAY008",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 47500,
    paymentDate: "2023-12-15",
    paymentMethod: "Online - Credit Card",
    transactionId: "CC123456789",
    semester: 2,
    academicYear: "2023-24",
    receiptNumber: "RCP-2023-002",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
  },
  // Previous year payments for testing
  {
    id: "PAY009",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 44000,
    paymentDate: "2023-08-20",
    paymentMethod: "UPI",
    transactionId: "UPI987654321",
    semester: 1,
    academicYear: "2023-24",
    receiptNumber: "RCP-2023-001",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
    feeBreakdown: {
      tuitionFee: 35000,
      libraryFee: 1800,
      labFee: 3500,
      sportsFee: 1200,
      developmentFee: 2000,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 0,
      total: 44000
    }
  },
  {
    id: "PAY010",
    studentEnrollment: "2025110001",
    studentName: "Arjun Mehta",
    amount: 46000,
    paymentDate: "2023-12-15",
    paymentMethod: "Credit Card",
    transactionId: "CC123456789",
    semester: 2,
    academicYear: "2023-24",
    receiptNumber: "RCP-2023-002",
    status: "Completed",
    paidFor: ["Tuition Fee", "Library Fee", "Lab Fee", "Sports Fee", "Development Fee", "Exam Fee"],
    feeBreakdown: {
      tuitionFee: 36500,
      libraryFee: 1800,
      labFee: 3800,
      sportsFee: 1200,
      developmentFee: 2200,
      examFee: 500,
      hostelFee: 0,
      messFee: 0,
      lateFee: 0,
      total: 46000
    }
  },
];

// Sample Student Login Data (for demonstration)
export const currentStudentLogin = {
  enrollment: "2025110001", // This would come from authentication
  name: "Arjun Mehta",
  department: "CSE",
  semester: 4,
  email: "arjun.mehta@college.com",
  phone: "9876543211",
  photo: "https://randomuser.me/api/portraits/men/11.jpg",
};