...
export default function EditNoticePage() {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            publishDate: noticeData.publishDate ? new Date(noticeData.publishDate).toISOString().split('T')[0] : '',
            expiryDate: noticeData.expiryDate ? new Date(noticeData.expiryDate).toISOString().split('T')[0] : '',
            isPinned: noticeData.isPinned || false,
            targetSemesters: noticeData.targetSemesters || [],
            targetBranches: noticeData.targetBranches || [],
            tags: (noticeData.tags || []).join(', ')
          });

          if (noticeData.attachmentUrl) {
            setCurrentAttachment({
              url: noticeData.attachmentUrl,
              name: noticeData.attachmentName
            });
          }
        } else {
          setErrors({ general: 'Notice not found' });
        }
      } catch (err) {
        console.error('Error fetching notice:', err);
        setErrors({ general: err.message || 'Failed to fetch notice' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const removeCurrentAttachment = () => {
    ...
    // Note: You might want to add logic to mark for deletion
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        router.push(`/list/notices/${noticeId}`);
      } else {
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Update notice information and content</p>
          </div>
          <button
            onClick={() => router.back()}
            ...
            <div className="mb-4 p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-900">Current: {currentAttachment.name}</p>
                    <p className="text-sm text-gray-500">Click to view current attachment</p>
                  </div>
                </div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
            ...
            >
              ...
              <span className="text-gray-600 text-center">
                {currentAttachment ? 'Upload new attachment to replace current' : 'Click to upload attachment'}<br />
